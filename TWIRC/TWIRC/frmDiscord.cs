...
namespace SayingsBot
{
    public partial class frmDiscord : Form
    {
        ...
        public frmDiscord(Logger newlogger, HarbBot bot)
        {
            RNGLogger = newlogger;
            HB = bot;
            InitializeComponent();
        ...
        private void frmDiscord_ForumClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void frmDiscord_SizeChanged(object sender, System.EventArgs e)
        {
            lbBanter.Width = this.Width;
            txt_Discord.Width = this.Width;
            btnConnect.Left = ((this.Width - btnConnect.Width) - 16);
            btnConnect.Top = ((this.Height - btnConnect.Height) - 40);
            //do channels befoer guild
            lbChannels.Top = btnConnect.Top - 62;
            lbGuilds.Top = lbChannels.Top;
            lbChannels.Left = lbGuilds.Width + 5;
        }
        ...
        private void SendMessage()
        {
            ...
            // do nothing
            if (string.IsNullOrWhiteSpace(this.txt_Discord.Text))
                return;
            ...
            _ = Task.Run(() => this.BotSendMessageCallback(this.txt_Discord.Text, this.SelectedChannel))
                .ContinueWith(t => this.txt_Discord.SetProperty(x => x.Text, ""));
        }
        ...
        private async Task BotThreadCallback()
        {
            ...
            // elements again
            this.btnConnect.SetProperty(x => x.Enabled, true);
            //this.btMsgSend.SetProperty(x => x.Enabled, true);
            this.txt_Discord.SetProperty(x => x.Enabled, true);
            ...
            this.txt_Discord.SetProperty(x => x.Enabled, false);
        ...
        private Task Bot_MessageCreated(MessageCreateEventArgs e)
        {
            ...
            {
                //Get the user's nickname and replace it with their twitch name if applicable
                string nick = e.Message.Author.Username;
                if (HB.commands.getUserFromAlias(e.Message.Author.Username) != null)
                    nick = HB.commands.getUserFromAlias(e.Message.Author.Username);
                if (HB.commands.getUserFromAlias(e.Message.Author.Id.ToString()) != null)
                    nick = HB.commands.getUserFromAlias(e.Message.Author.Id.ToString());
                HB.commands.addPoints(nick, 2);
                HB.commands.addAllTime(nick, 2);
            }
            string commandResponse = HB.commands.checkCommand(e.Channel.Name, e.Message.Author.Username, e.Message.Content);
            if (commandResponse != "ERROR" && commandResponse != null)
            {
                //sendMess(channel, response);
                Task.Run(() => this.BotSendMessageCallback(commandResponse, new BotChannel(e.Channel)));
            }
        ...
        {
            this.lbBanter.Items.Add(msg.user.Username + ": " +msg);
        ...
        private void btnConnect_Click(object sender, EventArgs e)
        {
            ...
            this.txt_Discord.Enabled = false;
            ...
            if (this.BotThread == null)
            {
                ...
                this.btnConnect.Text = "Stop";

                // create the bot container
                this.Bot = new Bot("MTg4ODAyMTAwNTg5ODg3NDg5.CjT7KA.BKKSsPcKjyomOFjeitHFk8N2TgU");
